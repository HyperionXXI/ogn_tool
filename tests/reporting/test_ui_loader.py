...
def test_build_ui_artifact_from_run_reads_files(tmp_path: Path) -> None:
    ...
    (run_dir / 'report.json').write_text(json.dumps(_canonical_report()), encoding='utf-8')
    ...
    assert 'network_summary' in artifact
    assert 'stations' in artifact
    assert 'metrics' in artifact
    assert 'debug' in artifact
