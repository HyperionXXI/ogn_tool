...
def build_dataset(engine: Any, dataset_mode: str = "NETWORK", station_id: str | None = None) -> dict:
    """Build legacy analysis dataset dict from raw packets."""
    from .rf_engine_dataset_builder import build_analysis_dataset_impl

    return build_analysis_dataset_impl(engine, dataset_mode=dataset_mode, station_id=station_id)
...
def normalize_observations(packet_rows: Iterable[Dict]) -> pd.DataFrame:
    """Normalize packet rows while preserving backward-compatible aliases."""
...
def _build_rf_observations_frame(packet_rows: Iterable[Dict]) -> pd.DataFrame:
    normalized_df = normalize_observations(packet_rows)
...
def build_observations(packet_rows: Iterable[Dict]) -> List[RFObservationVector]:
    ...
    if obs_df is None or obs_df.empty:
        rows = normalize_observations(packet_rows)
