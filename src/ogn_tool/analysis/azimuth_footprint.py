...
def compute_azimuth_footprint(
    ...
    if "lat" not in df_packets.columns or "lon" not in df_packets.columns or "distance_km" not in df_packets.columns:
        return {"implemented": False, "summary": {"reason": "missing_columns"}, "data": None}

    lat = np.radians(pd.to_numeric(df_packets["lat"], errors="coerce").to_numpy())
    lon = np.radians(pd.to_numeric(df_packets["lon"], errors="coerce").to_numpy())
