...
def load_rf_signature(run_path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(signature, dict):
        raise SystemExit(f"rf_signature missing or invalid in {report_path}")
    return signature
...
def main() -> None:
    ...
    sig_a = load_rf_signature(run_a_path)
    sig_b = load_rf_signature(run_b_path)
